...
class TransposedConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=1, op=1, g=1, d=1, act=True):
        super().__init__()
        self.TransposedConv = nn.ConvTranspose2d(c1, c2, k, s, p, op, groups=g, dilation=d, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()

    def forward(self, x):
        return self.act(self.bn(self.TransposedConv(x)))

    def forward_fuse(self, x):
        return self.act(self.TransposedConv(x))
